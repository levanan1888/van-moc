...
class CategoryRepository extends RepositoriesAbstract implements CategoryInterface
{
     ...
     * {@inheritDoc}
     */
    public function getCategories(array $select, array $orderBy, $condition = [])
    {
        $data = $this->model;
        if (empty($condition['status'])) {
            $data = $data->where('status', BaseStatusEnum::PUBLISHED);
        }
        $data = $data->with('slugable')
        ->select($select);
     ...
     * {@inheritDoc}
     */
    public function getPopularCategories(int $limit, array $with = ['slugable'], array $withCount = ['products'])
    {
        $data = $this->model
            ->with($with)
            ->withCount($withCount)
