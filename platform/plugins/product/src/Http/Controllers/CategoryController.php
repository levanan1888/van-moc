...
use Botble\Product\Forms\CategoryForm;
...
class CategoryController extends BaseController
{
    ...
    {
        page_title()->setTitle(trans('plugins/product::categories.menu'));
        ...
        $categories->load('slugable')->loadCount('products');
    ...
    {
        page_title()->setTitle(trans('plugins/product::categories.create'));
    ...
    public function store(CategoryRequest $request, BaseHttpResponse $response)
    {
        ...
        event(new CreatedContentEvent(PCATEGORY_MODULE_SCREEN_NAME, $request, $category));
        ...
        return $response
            ->setPreviousUrl(route('pcategories.index'))
            ->setNextUrl(route('pcategories.edit', $category->id))
    ...
    public function edit($id, FormBuilder $formBuilder, Request $request, BaseHttpResponse $response)
    {
        ...
        page_title()->setTitle(trans('plugins/product::categories.edit') . ' "' . $category->name . '"');
    ...
    public function update($id, CategoryRequest $request, BaseHttpResponse $response)
    {
        ...
        event(new UpdatedContentEvent(PCATEGORY_MODULE_SCREEN_NAME, $request, $category));
    ...
    public function destroy(Request $request, $id, BaseHttpResponse $response)
    {
        try {
            ...
            event(new DeletedContentEvent(PCATEGORY_MODULE_SCREEN_NAME, $request, $category));
    ...
    {
        if (!$model) {
            $form->setUrl(route('pcategories.create'));
        }

        if (!Auth::user()->hasPermission('pcategories.create') && !$model) {
    ...
    {
        return array_merge([
            'canCreate' => Auth::user()->hasPermission('pcategories.create'),
            'canEdit' => Auth::user()->hasPermission('pcategories.edit'),
            'canDelete' => Auth::user()->hasPermission('pcategories.destroy'),
            'createRoute' => 'pcategories.create',
            'editRoute' => 'pcategories.edit',
            'deleteRoute' => 'pcategories.destroy',
        ], $options);
    }
}
