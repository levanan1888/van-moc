...
class Meta implements MetaContract
{
    ...
    protected function renderLink()
    {
        ...
        return '<link rel="' . $this->getName(false) . '" href="' . $this->addTrailingSlash($content) . '">';
    }
    ...
    protected function renderMeta()
    {
        ...
        $content = $this->getContent();
        $output[] = 'content="' . $this->addTrailingSlash($content) . '"';
     ...
     * @return string
     */
    protected function addTrailingSlash($content)
    {
        if (substr($content, -1) === '/') {
            return $content;
        }

        $isLink = filter_var($content, FILTER_VALIDATE_URL) !== false;
        if (!$isLink) {
            return $content;
        }

        $imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp'];
        $urlExtension = pathinfo($content, PATHINFO_EXTENSION);
        if (in_array(strtolower($urlExtension), $imageExtensions)) {
            return $content;
        }
