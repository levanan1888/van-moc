...
class Sitemap
{
    ...
    public function generate(string $format = 'xml'): array
    {
        ...
        array_multisort(array_column($items, 'priority'), SORT_DESC, array_column($items, 'lastmod'), SORT_DESC, $items);

        switch ($format) {
            case 'ror-rss':
                return ['content' => $this->view->make('packages/sitemap::ror-rss', ['items' => $items, 'channel' => $channel, 'style' => $style])->render(), 'headers' => ['Content-type' => 'text/rss+xml; charset=utf-8']];
            case 'ror-rdf':
                return ['content' => $this->view->make('packages/sitemap::ror-rdf', ['items' => $items, 'channel' => $channel, 'style' => $style])->render(), 'headers' => ['Content-type' => 'text/rdf+xml; charset=utf-8']];
            case 'html':
                return ['content' => $this->view->make('packages/sitemap::html', ['items' => $items, 'channel' => $channel, 'style' => $style])->render(), 'headers' => ['Content-type' => 'text/html; charset=utf-8']];
            case 'txt':
                return ['content' => $this->view->make('packages/sitemap::txt', ['items' => $items, 'style' => $style])->render(), 'headers' => ['Content-type' => 'text/plain; charset=utf-8']];
            case 'sitemapindex':
            ...
            default:
                return ['content' => $this->view->make('packages/sitemap::' . $format, ['items' => $items, 'style' => $style])->render(), 'headers' => ['Content-type' => 'text/xml; charset=utf-8']];
        }
    }
